...
QUERY3="GRANT ALL PRIVILEGES ON ${MYSQL_DATABASE}.* TO '${MYSQL_USER}'@'%'; FLUSH PRIVILEGES;"
QUERY4="USE ${MYSQL_DATABASE}; CREATE TABLE IF NOT EXISTS info (id INT AUTO_INCREMENT PRIMARY KEY, f_name VARCHAR(30) NOT NULL, l_name VARCHAR(30) NOT NULL, pho_no INT NOT NULL);"
QUERY5="ALTER USER 'root' IDENTIFIED WITH mysql_native_password BY '${MYSQL_ROOT_PASSWORD}'; FLUSH PRIVILEGES;"
QUERY6="ALTER USER Open in Remote - Containers IDENTIFIED WITH mysql_native_password BY '${MYSQL_PASSWORD}'; FLUSH PRIVILEGES;"
